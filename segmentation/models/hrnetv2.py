...
__C = AttrDict()
__C.MODEL = AttrDict()
__C.MODEL.BNFUNC = nn.BatchNorm2d #apex.parallel.SyncBatchNorm
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, num_classes=10,**kwargs):
        ...
        self.classification_inchannels = np.int(np.sum(cfg.MODEL.OCR_EXTRA.STAGE4.NUM_CHANNELS))

        self.classification_layer = self._make_classification_layer(
            in_channels=self.classification_inchannels,
    ...
    def _make_classification_layer(self,in_channels,out_channels):
        return nn.Sequential(
            nn.ConvTranspose2d(in_channels,in_channels//2,3,stride=2,padding=1,bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels//2,out_channels,3,stride=1,padding=2,bias=False)
        )
    ...
    def forward(self, x_in):
        ...
        out = self.classification_layer(feats)
        # resize 
        
        out = F.interpolate(out, size=(x_in.size(2),x_in.size(3)),
                            mode='bilinear',align_corners=align_corners)
        out = F.sigmoid(out)

        return out

    def init_weights(self,pretrained=False,weights=""):
        ...
        if pretrained:
            pretrained_path = os.path.join(weights, 'hrnetv2_w48_imagenet_pretrained.pth')
            pretrained_dict = torch.load(pretrained_path,
                                         map_location={'cuda:0': 'cpu'})
            logging.info('=> loading pretrained model {}'.format(pretrained_path))
            ...
            raise RuntimeError('No such file {}'.format(pretrained_path))
